...
from models.self_modules import DeformBottleneck
from models.se_module import SELayer
from models.DaNet import _DAHead
...
class SelfHigherResolutionNet(nn.Module):

    def __init__(self, **kwargs):
        self.inplanes = 64
        super(SelfHigherResolutionNet, self).__init__()
        self.make_baseline()
        self.backbone_channels = 32
        #self.init_weights()
    ...
    def make_baseline(self):
        ...
        self.transition3 = self._make_transition_layer(
            pre_stage_channels, num_channels)
        self.stage4, pre_stage_channels = self._make_stage(
            self.stage4_cfg, num_channels, multi_scale_output=False)  # stage4输出 [32,128,128]  128是输出的1/4

        # se模块
        self.se = SELayer(self.stage2_cfg['NUM_CHANNELS'][0], 4)
        # Da模块
        # self.Da = _DAHead(in_channels=self.stage2_cfg['NUM_CHANNELS'][0], nclass=self.stage2_cfg['NUM_CHANNELS'][0])
    ...
    def forward(self, x):
        ...
        # da_list = self.Da(x)  # list有3个，第一个是sum,第二个是c,第三个是p
        # x = da_list[0]
        return x
    def init_weights(self):
        for m in self.modules():
...
if __name__ == '__main__':
    ...
    model = SelfHigherResolutionNet()
    a=model(torch.rand(2,512,512,3))  # a [2,32,128,128]
    for i in a:
        print(i.shape)